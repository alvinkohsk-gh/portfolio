...
export type AnnouncementsDebug = Record<
  string,
  {
    nasdaq?: SourceDebug;
...
/** Scans every requested symbol across SEC EDGAR / Nasdaq (US) or Yahoo
 * Finance / SGX / Google News (SGX), plus stock splits for all symbols.
 * Shared by /api/announcements (on-demand, per-viewer refresh) and the
 * /api/digest cron (scheduled email digest) so both stay on one
 * implementation instead of drifting apart. */
export async function fetchAnnouncementsForSymbols(
  symbols: string[],
  names: string[] = []
): Promise<AnnouncementsResult> {
  if (symbols.length === 0) {
    return { announcements: {}, errors: [], debug: {} };
  }

  const announcements: Record<string, Announcement[]> = {};
  const errors: string[] = [];
  const debug: AnnouncementsDebug = {};
  ...
  await Promise.all([
    ...symbols.map(async (symbol) => {
      try {
        const items: Announcement[] = [];
        const symbolDebug: AnnouncementsDebug[string] = {};
